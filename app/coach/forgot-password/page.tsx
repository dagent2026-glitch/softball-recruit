...
export default function CoachForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/coach/request-password-reset', {
        method: 'POST',
      ...
      setMessage(data.message || 'If a coach account with that email exists, a password reset link has been sent.');
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#18181b] flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-[#18181b] mt-2">Coach Forgot Password</h1>
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          <Link href="/coach/login" className="text-lime-700 font-semibold hover:underline">Back to login</Link>
        </p>
      </div>
    </div>
  );
